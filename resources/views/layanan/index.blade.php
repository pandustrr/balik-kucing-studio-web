        ...
        <div class="grid md:grid-cols-3 gap-8 reveal-group">
            ...
            <div class="group relative p-12 glass rounded-[48px] reveal-item transition-all duration-700 hover:-translate-y-4 hover:shadow-[0_40px_80px_-20px_rgba(244,124,32,0.15)] overflow-hidden">
                ...
                <div class="relative z-10 space-y-8">
                    ...
                    <div class="space-y-4">
                        <h4 class="text-3xl font-heading font-black tracking-tight uppercase">{{ $category->name }}</h4>
                        ...
                        <ul class="space-y-2 pt-4 border-t border-foreground/5 dark:border-white/5">
                            @foreach($category->pricelists->take(3) as $pricelist)
                            ...
                            @endforeach
                            @if($category->pricelists->count() > 3)
                            <li class="flex items-center gap-2 text-sm opacity-40 italic">
                                <svg class="w-4 h-4 text-bk-orange" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                </svg>
                                +{{ $category->pricelists->count() - 3 }} layanan lainnya
                            </li>
                            @endif
                        </ul>
                        @else
                        <p class="text-xs opacity-40 italic pt-4 border-t border-foreground/5 dark:border-white/5">Belum ada paket layanan</p>
                        @endif
                    </div>

                    <a href="{{ route('contact') }}" class="inline-flex items-center gap-2 font-black text-xs tracking-widest uppercase text-bk-orange group-hover:gap-4 transition-all cursor-pointer">
                        Lihat Detail
            ...
            @empty
            <!-- Fallback: Show default cards if no categories -->
            <!-- Card 1 - Ya Desain -->
            <div class="group relative p-12 glass rounded-[48px] reveal-item transition-all duration-700 hover:-translate-y-4 hover:shadow-[0_40px_80px_-20px_rgba(244,124,32,0.15)] overflow-hidden">
                <div class="absolute -right-8 -top-8 w-32 h-32 bg-bk-orange/5 rounded-full blur-3xl group-hover:bg-bk-orange/20 transition-all duration-700"></div>
